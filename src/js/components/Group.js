...
export default class Group extends Component {
  constructor() {
    super();
    this.onGroupResponse = this.onGroupResponse.bind(this);
  ...
  componentWillReceiveProps(newProps) {
    if (newProps.id !== this.props.id) {
      this.getGroup(newProps.id);
    }
  }

  onGroupResponse(result) {
    const group = result[0];
    this.setState({ group, owners: [], error: undefined });
    ...
    if (config.scopes.groups.attributes.owner) {
      ...
      if (owner) {
        ...
        fetch(`/ldap/${query}`, options)
          ...
          .then(response => this.setState({ owners: response, error: undefined, busy: false }))
          .catch(error => this.setState({ owners: [], error, busy: false }));
      }
    }
  }
    ...
    if (busy) {
      ...
      ownerItems = owners.map((item, index) => (
        <PersonListItem
          ...
          item={item}
          first={index === 0}
          onClick={() => this.onSelectOwner(item)}
        />
      ));
    }
